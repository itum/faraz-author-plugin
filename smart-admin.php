...
function build_template_prompt($form_data) {
    ...
    if (isset($form_data['destination'])) {
        ...
        $prompt = "ایجاد یک راهنمای سفر جامع و کاربردی برای سفر به $destination با رعایت اصول سئو و بهینه‌سازی برای رنک مث.

موضوع: راهنمای کامل سفر به $destination

اطلاعات سفر:
- فصل سفر: $travel_season
- نوع سفر: $travel_type
- مدت پیشنهادی: $travel_duration
- سطح بودجه: $budget_level

ساختار مقاله:
1. عنوان اصلی (H1): راهنمای کامل سفر به $destination: تجربه‌ای فراموش‌نشدنی (یا عنوانی جذاب دیگر)

2. بدنه مقاله باید شامل این بخش‌های اصلی باشد (هر بخش باید با تیتر H2 مشخص شود، با فرمت ## عنوان):
   - معرفی $destination (تاریخچه، فرهنگ، چرا باید به آنجا سفر کرد)
   - برنامه‌ریزی سفر (زمان مناسب سفر، مدت اقامت، بودجه‌بندی)
   - نحوه رسیدن به $destination (مسیرهای دسترسی، فرودگاه‌ها، ویزا)
   - حمل و نقل در $destination (گزینه‌های محلی، کرایه خودرو، اپلیکیشن‌های کاربردی)
   - اقامت در $destination (محله‌های مناسب، انواع اقامتگاه‌ها، قیمت‌ها)
   - جاذبه‌های گردشگری برتر (معرفی جاذبه‌ها با جزئیات کامل)
   - تجربه‌های منحصربفرد (فعالیت‌های خاص، تورها، رویدادها)
   - غذا و نوشیدنی (غذاهای محلی، رستوران‌های برتر)
   - خرید در $destination (بازارها، مراکز خرید، سوغاتی‌ها)
   - نکات مهم و توصیه‌ها (ایمنی، آداب فرهنگی، صرفه‌جویی)
...
3. نکات مهم نگارشی:
   ...
   - از لحن دوستانه و صمیمی استفاده کن
   - از تجربیات واقعی و داستان‌های جذاب استفاده کن
   - از جملات کوتاه و پاراگراف‌های کم‌حجم استفاده کن
   - از لیست‌های شماره‌دار و بولت‌پوینت برای خوانایی بهتر استفاده کن
   - از آمار و ارقام واقعی استفاده کن
   - از نقل قول‌های مسافران یا منابع معتبر استفاده کن
   - به چالش‌ها و مشکلات احتمالی سفر هم اشاره کن

4. بهینه‌سازی محتوا:
   - طول مقاله: حداقل ۴۰۰۰ کلمه
   - استفاده از هدینگ‌های H2 برای بخش‌های اصلی مقاله (با فرمت ## عنوان) و H3 برای زیربخش‌ها (با فرمت ### عنوان) به صورت سلسله مراتبی
   - پیشنهاد برای افزودن تصاویر جذاب با متن ALT مناسب
   - پیشنهاد برای لینک‌دهی داخلی و خارجی به منابع مرتبط
   
5. نکته مهم: در ابتدای مقاله از عبارت «مقدمه» یا هر عنوان مشابه استفاده نکن. مستقیماً با یک پاراگراف جذاب شروع کن که خواننده را به ادامه مطلب ترغیب کند.

لطفاً محتوایی تولید کن که کاملاً طبیعی به نظر برسد و مانند یک راهنمای سفر واقعی نوشته شده توسط یک متخصص گردشگری باشد.";
        ...
        $prompt = "ایجاد یک مقاله جامع و کاربردی در مورد $food_topic با رعایت اصول سئو و بهینه‌سازی برای رنک مث.
...
ساختار مقاله:
1. عنوان اصلی (H1): $food_topic: دستور پخت اصیل و حرفه‌ای (یا عنوانی جذاب دیگر)

2. بدنه مقاله باید شامل این بخش‌های اصلی باشد (هر بخش باید با تیتر H2 مشخص شود، با فرمت ## عنوان):
   - معرفی غذا (تاریخچه، ریشه فرهنگی، محبوبیت)
   - ارزش غذایی و فواید سلامتی (کالری، مواد مغذی، خواص)
   - مواد لازم (لیست کامل با مقادیر دقیق)
   - تجهیزات مورد نیاز (وسایل آشپزی ضروری)
   - مراحل آماده‌سازی (قدم به قدم با جزئیات)
   - نکات کلیدی و ترفندهای حرفه‌ای (رازهای موفقیت)
   - روش‌های سرو و تزیین (پیشنهادات خلاقانه)
   - غذاها و نوشیدنی‌های مکمل (پیشنهادات ترکیب)
   - تنوع و تغییرات (نسخه‌های منطقه‌ای، مدرن، گیاهی)
   - نگهداری و گرم کردن مجدد (روش‌های صحیح)
   - پرسش‌های متداول (حداقل ۸ سوال و پاسخ)
   - نتیجه‌گیری و جمع‌بندی

3. نکات مهم نگارشی:
   - از کلمات کلیدی مرتبط با $food_topic به صورت طبیعی استفاده کن
   - از عبارات و اصطلاحات تخصصی آشپزی استفاده کن
   - از لحن گرم، صمیمی و اشتیاق‌برانگیز استفاده کن
   - از توصیفات حسی (طعم، بو، بافت) استفاده کن
   - از جملات کوتاه و پاراگراف‌های کم‌حجم استفاده کن
   - از لیست‌های شماره‌دار و بولت‌پوینت برای خوانایی بهتر استفاده کن
   - از داستان‌های کوتاه و جذاب مرتبط با غذا استفاده کن
   - از نقل قول‌های سرآشپزها یا منابع معتبر استفاده کن
   - به نکات ایمنی و بهداشتی هم اشاره کن

4. بهینه‌سازی محتوا:
   - طول مقاله: حداقل ۳۰۰۰ کلمه
   - استفاده از هدینگ‌های H2 برای بخش‌های اصلی مقاله (با فرمت ## عنوان) و H3 برای زیربخش‌ها (با فرمت ### عنوان) به صورت سلسله مراتبی
   - پیشنهاد برای افزودن تصاویر جذاب با متن ALT مناسب
   - پیشنهاد برای لینک‌دهی داخلی و خارجی به منابع مرتبط
   
5. نکته مهم: در ابتدای مقاله از عبارت «مقدمه» یا هر عنوان مشابه استفاده نکن. مستقیماً با یک پاراگراف اشتهاآور و جذاب شروع کن که خواننده را به ادامه مطلب ترغیب کند.

لطفاً محتوایی تولید کن که کاملاً طبیعی به نظر برسد و مانند یک دستور پخت واقعی نوشته شده توسط یک سرآشپز حرفه‌ای باشد.";
        ...
        $prompt = "ایجاد یک مقاله استاندارد و بهینه‌شده برای SEO در مورد $main_topic با تمرکز بر کلمه کلیدی $focus_keyword.
...
ساختار مقاله:
1. عنوان اصلی (H1): عنوانی جذاب و گیرا که شامل کلمه کلیدی $focus_keyword باشد (حداکثر ۶۰ کاراکتر)

2. بدنه مقاله باید شامل این بخش‌ها باشد:
   - پاراگراف شروعی جذاب (بدون استفاده از عنوان «مقدمه») که کلمه کلیدی اصلی در آن به کار رفته باشد
   - حداقل ۵ بخش اصلی که هر کدام با تیتر H2 مشخص شود (با فرمت ## عنوان) و زیربخش‌های مرتبط با H3 (با فرمت ### عنوان)
   - بخش پرسش و پاسخ (FAQ) با حداقل ۳ سوال متداول
   - جمع‌بندی و نتیجه‌گیری
   
   توجه: فهرست مطالب را در ابتدای مقاله قرار نده، فقط محتوای اصلی تولید کن.

3. نکات مهم نگارشی:
   - استفاده از کلمه کلیدی اصلی در عنوان، پاراگراف اول، حداقل یک H2 یا H3 و در متن مقاله با تراکم مناسب (۱٪ تا ۲٪)
   - استفاده از کلمات کلیدی مرتبط و هم‌معنا در سراسر متن
   - استفاده از جملات کوتاه و پاراگراف‌های کم‌حجم (حداکثر ۴-۵ خط)
   - استفاده از لیست‌های شماره‌دار و بولت‌پوینت برای خوانایی بهتر
   - استفاده از نقل قول‌ها و آمار و ارقام معتبر
   - استفاده از مثال‌های عملی و کاربردی
   - لحن روان، حرفه‌ای و قابل فهم
   - متن‌های انگلیسی را چپ‌چین کن و از علامت‌های --- استفاده نکن

4. بهینه‌سازی محتوا:
   - پیشنهاد برای عنوان متا (Meta Title): حداکثر ۶۰ کاراکتر، شامل کلمه کلیدی اصلی
   - پیشنهاد برای توضیحات متا (Meta Description): حداکثر ۱۶۰ کاراکتر، شامل کلمه کلیدی اصلی
   - پیشنهاد برای آدرس URL: کوتاه و شامل کلمه کلیدی اصلی
   - پیشنهاد برای افزودن تصاویر با متن ALT مناسب
   - پیشنهاد برای لینک‌دهی داخلی (حداقل ۲ لینک) و خارجی (حداقل ۱ لینک)
   
5. نکته مهم: در ابتدای مقاله از عبارت «مقدمه» یا هر عنوان مشابه استفاده نکن. مستقیماً با یک پاراگراف جذاب شروع کن که کلمه کلیدی اصلی را شامل شود و خواننده را به ادامه مطلب ترغیب کند.

لطفاً محتوایی تولید کن که کاملاً طبیعی به نظر برسد و توسط یک نویسنده انسانی نوشته شده باشد، نه هوش مصنوعی.";
        ...
        $prompt = "ایجاد یک مقاله بنیادی (Pillar Page) جامع و کامل در مورد $main_topic برای مخاطبان $target_audience.

موضوع: $main_topic
مخاطب هدف: $target_audience
نوع محتوا: $content_goal
طول مقاله: حداقل ۲۵۰۰ کلمه

ساختار مقاله:
1. عنوان اصلی (H1): عنوانی جذاب و گیرا که موضوع اصلی را به خوبی منعکس کند (حداکثر ۶۰ کاراکتر)

2. بدنه مقاله باید شامل این بخش‌ها باشد:
   - پاراگراف شروعی قوی (بدون استفاده از عنوان «مقدمه») که مشکل یا نیاز مخاطب را مطرح کند و وعده راه‌حل دهد
   - حداقل ۵ بخش اصلی که هر کدام با تیتر H2 مشخص شود (با فرمت ## عنوان) و هر جنبه مهم موضوع را پوشش دهد
   - زیربخش‌های مرتبط با H3 (با فرمت ### عنوان) برای هر بخش اصلی
   - بخش پرسش و پاسخ (FAQ) با حداقل ۵ سوال متداول
   - جمع‌بندی و نتیجه‌گیری با دعوت به اقدام (CTA)
   
   توجه: فهرست مطالب را در ابتدای مقاله قرار نده، فقط محتوای اصلی تولید کن.

3. نکات مهم نگارشی:
   - استفاده از لحن معتبر و حرفه‌ای اما روان و قابل فهم
   - استفاده از جملات کوتاه و پاراگراف‌های کم‌حجم (حداکثر ۳-۴ جمله)
   - استفاده از لیست‌های شماره‌دار و بولت‌پوینت برای خوانایی بهتر
   - استفاده از نقل قول‌ها و آمار و ارقام معتبر
   - استفاده از مثال‌های عملی و کاربردی
   - استفاده از کلمات کلیدی مرتبط به صورت طبیعی (بدون تکرار بیش از حد)
   - متن‌های انگلیسی را چپ‌چین کن و از علامت‌های --- استفاده نکن

4. بهینه‌سازی محتوا:
   - پیشنهاد برای عنوان متا (Meta Title): حداکثر ۶۰ کاراکتر
   - پیشنهاد برای توضیحات متا (Meta Description): حداکثر ۱۶۰ کاراکتر
   - پیشنهاد برای آدرس URL: کوتاه و مرتبط با موضوع
   - پیشنهاد برای افزودن تصاویر با متن ALT مناسب
   - پیشنهاد برای لینک‌دهی داخلی (حداقل ۳ لینک) و خارجی (حداقل ۲ لینک)
   
5. نکته مهم: در ابتدای مقاله از عبارت «مقدمه» یا هر عنوان مشابه استفاده نکن. مستقیماً با یک پاراگراف قدرتمند شروع کن که مشکل یا نیاز مخاطب را مطرح کند و خواننده را به ادامه مطلب ترغیب کند.

لطفاً محتوایی تولید کن که کاملاً طبیعی به نظر برسد و توسط یک متخصص حرفه‌ای در این حوزه نوشته شده باشد.";
        ...
        $prompt = "ایجاد یک راهنمای عملی قدم به قدم برای $how_to_topic مناسب برای کاربران $skill_level.

موضوع: $how_to_topic
سطح مخاطب: $skill_level
ابزارهای مورد نیاز: $required_tools
طول مقاله: حداقل ۱۵۰۰ کلمه

ساختار راهنما:
1. عنوان اصلی (H1): راهنمای کامل و قدم به قدم $how_to_topic (یا عنوانی مشابه و جذاب)

2. بدنه راهنما باید شامل این بخش‌ها باشد:
   - پاراگراف شروعی (بدون استفاده از عنوان «مقدمه») که هدف راهنما و نتیجه نهایی را توضیح دهد
   - بخش «آنچه نیاز دارید» شامل لیست ابزارها، مواد یا نرم‌افزارهای مورد نیاز
   - حداقل ۵ مرحله اصلی که هر کدام با تیتر H2 مشخص شود (با فرمت ## مرحله ۱: آماده‌سازی)
   - توضیحات دقیق و جزئی برای هر مرحله، ترجیحاً با لیست‌های شماره‌دار
   - بخش «نکات حرفه‌ای» یا «اشتباهات رایج» بعد از هر چند مرحله
...
3. نکات مهم نگارشی:
   ...
   - استفاده از لیست‌های شماره‌دار و بولت‌پوینت برای خوانایی بهتر
   - توضیح دقیق هر مرحله با فرض اینکه کاربر هیچ دانش قبلی ندارد
   - استفاده از لحن دوستانه، حمایتی و تشویق‌کننده
   - پیشنهاد برای افزودن تصاویر یا ویدیوهای توضیحی در هر مرحله
   - متن‌های انگلیسی را چپ‌چین کن و از علامت‌های --- استفاده نکن

4. بهینه‌سازی محتوا:
   - استفاده از کلمات کلیدی مرتبط با $how_to_topic به صورت طبیعی
   - استفاده از هدینگ‌های H2 برای بخش‌های اصلی مقاله (با فرمت ## عنوان) و H3 برای زیربخش‌ها (با فرمت ### عنوان) به صورت سلسله مراتبی
   - پیشنهاد برای لینک‌دهی داخلی و خارجی به منابع مرتبط
   - استفاده از Schema HowTo برای بهینه‌سازی SEO
   
5. نکته مهم: در ابتدای راهنما از عبارت «مقدمه» استفاده نکن. مستقیماً با یک پاراگراف شروع کن که هدف راهنما را توضیح دهد و کاربر را برای یادگیری آماده کند.

لطفاً محتوایی تولید کن که کاملاً طبیعی به نظر برسد و مانند یک آموزش واقعی نوشته شده توسط یک متخصص باشد.";
        ...
        $prompt = "ایجاد یک مقاله لیستی جذاب و کاربردی با عنوان $list_topic شامل $list_count مورد برتر.

موضوع: $list_topic
تعداد آیتم‌ها: $list_count
معیار رتبه‌بندی: $list_criteria
طول مقاله: حداقل ۱۸۰۰ کلمه

ساختار مقاله:
1. عنوان اصلی (H1): $list_count $list_topic (یا عنوانی جذاب و کلیک‌خور مشابه)

2. بدنه مقاله باید شامل این بخش‌ها باشد:
   - پاراگراف شروعی جذاب (بدون استفاده از عنوان «مقدمه») که اهمیت موضوع را نشان دهد و مخاطب را به خواندن ادامه مطلب ترغیب کند
   - توضیح کوتاهی درباره معیارهای انتخاب و رتبه‌بندی آیتم‌ها
   - لیست اصلی با $list_count آیتم، هر کدام با یک تیتر H2 (با فرمت ## ۱. [نام آیتم])
   - برای هر آیتم، این بخش‌ها را پوشش دهید:
     * معرفی کوتاه آیتم
     * ویژگی‌های کلیدی (به صورت بولت‌پوینت)
     * بهترین کاربرد و مناسب برای چه کسانی
     * قیمت‌گذاری یا دسترسی
   - یک یا دو آیتم ویژه یا کمتر شناخته شده در انتهای لیست
   - جدول مقایسه‌ای ساده برای مقایسه آیتم‌های اصلی
   - جمع‌بندی و توصیه نهایی
   
   توجه: فهرست مطالب را در ابتدای مقاله قرار نده، فقط محتوای اصلی تولید کن.

3. نکات مهم نگارشی:
   - استفاده از لحن جذاب، پرانرژی و صمیمی
   - استفاده از توصیفات قانع‌کننده و ملموس
   - استفاده از کلمات کلیدی مرتبط مانند «بهترین»، «برترین»، «نقد و بررسی»
   - استفاده از جملات کوتاه و پاراگراف‌های کم‌حجم
   - استفاده از لیست‌های شماره‌دار و بولت‌پوینت برای خوانایی بهتر
   - استفاده از مثال‌های عملی و کاربردی
   - متن‌های انگلیسی را چپ‌چین کن و از علامت‌های --- استفاده نکن
...
4. بهینه‌سازی محتوا:
   - استفاده از کلمات کلیدی مرتبط با $list_topic به صورت طبیعی
   - استفاده از هدینگ‌های H2 برای بخش‌های اصلی مقاله (با فرمت ## عنوان) و H3 برای زیربخش‌ها (با فرمت ### عنوان) به صورت سلسله مراتبی
   - پیشنهاد برای افزودن تصاویر مرتبط با هر آیتم
   - پیشنهاد برای لینک‌دهی داخلی و خارجی به منابع مرتبط
   
5. نکته مهم: در ابتدای مقاله از عبارت «مقدمه» استفاده نکن. مستقیماً با یک پاراگراف جذاب شروع کن که اهمیت موضوع را نشان دهد و مخاطب را به خواندن ادامه مطلب ترغیب کند.

لطفاً محتوایی تولید کن که کاملاً طبیعی به نظر برسد و مانند یک مقاله لیستی واقعی نوشته شده توسط یک وبلاگ‌نویس حرفه‌ای باشد.";
        ...
        $prompt = "ایجاد یک مقاله تخصصی جامع و کاربردی در مورد $tech_topic با رعایت اصول سئو و بهینه‌سازی برای رنک مث.
...
ساختار مقاله:
1. عنوان اصلی (H1): $tech_topic: راهنمای کامل و تخصصی (یا عنوانی جذاب دیگر)

2. بدنه مقاله باید شامل این بخش‌های اصلی باشد (هر بخش باید با تیتر H2 مشخص شود، با فرمت ## عنوان):
   - معرفی $tech_topic (تاریخچه، اهمیت، کاربردها)
   - پیش‌نیازها و آماده‌سازی محیط (ابزارها، نصب، پیکربندی)
   - مفاهیم پایه و اصول اولیه (اصطلاحات، ساختار، منطق)
   - پیاده‌سازی عملی (نمونه کدها، پروژه‌های کاربردی)
   - بهترین شیوه‌ها و الگوهای طراحی (Best Practices)
   - عیب‌یابی و رفع مشکلات رایج (Debugging)
   - بهینه‌سازی و عملکرد (Performance Optimization)
   - امنیت و نکات مهم (Security Considerations)
   - آینده و روندهای پیش‌رو (Future Trends)
   - منابع و مراجع تکمیلی (References, Documentation)
   - بخش کد نمونه (حداقل ۳ نمونه کد کاربردی با توضیحات کامل)
   - پرسش‌های متداول (حداقل ۸ سوال و پاسخ)
   - نتیجه‌گیری و جمع‌بندی

3. نکات مهم نگارشی:
   - از کلمات کلیدی مرتبط با $tech_topic به صورت طبیعی استفاده کن
   - از عبارات و اصطلاحات تخصصی فناوری اطلاعات استفاده کن
   - از لحن تخصصی اما قابل فهم برای مخاطب هدف استفاده کن
   - از مثال‌های عملی و کدهای کاربردی استفاده کن
   - از جملات کوتاه و پاراگراف‌های کم‌حجم استفاده کن
   - از لیست‌های شماره‌دار و بولت‌پوینت برای خوانایی بهتر استفاده کن
   - از آمار و ارقام مرتبط با صنعت IT استفاده کن
   - از مثال‌های عملی و تجربیات واقعی استفاده کن
   - به نکات امنیتی و بهترین شیوه‌ها اشاره کن
   - متن‌های انگلیسی را چپ‌چین کن و از علامت‌های --- استفاده نکن
   - کدها و دستورات را در بلوک‌های کد جداگانه قرار بده

4. بهینه‌سازی محتوا:
   - طول مقاله: حداقل ۴۰۰۰ کلمه
   - استفاده از هدینگ‌های H2 برای بخش‌های اصلی مقاله (با فرمت ## عنوان) و H3 برای زیربخش‌ها (با فرمت ### عنوان) به صورت سلسله مراتبی
   - پیشنهاد برای افزودن تصاویر از رابط‌ها، نمودارها، دیاگرام‌ها
   - پیشنهاد برای لینک‌دهی داخلی و خارجی به منابع مرتبط
   - فهرست مطالب را در ابتدای مقاله قرار نده، فقط محتوای اصلی تولید کن
   
5. نکته مهم: در ابتدای مقاله از عبارت «مقدمه» یا هر عنوان مشابه استفاده نکن. مستقیماً با یک پاراگراف جذاب شروع کن که خواننده را به ادامه مطلب ترغیب کند.

لطفاً محتوایی تولید کن که کاملاً طبیعی به نظر برسد و مانند یک مقاله تخصصی واقعی نوشته شده توسط یک متخصص فناوری اطلاعات باشد.";
        ...
        $prompt = "ایجاد یک مقاله مقایسه‌ای دقیق و منصفانه بین $item1 و $item2 برای کمک به خوانندگان در تصمیم‌گیری آگاهانه.

موضوع: مقایسه $item1 و $item2
معیارهای مقایسه: $comparison_criteria
طول مقاله: حداقل ۲۰۰۰ کلمه

ساختار مقاله:
1. عنوان اصلی (H1): $item1 در مقابل $item2: کدام برای شما مناسب‌تر است؟ (یا عنوانی مشابه و جذاب)

2. بدنه مقاله باید شامل این بخش‌ها باشد:
   - پاراگراف شروعی جذاب (بدون استفاده از عنوان «مقدمه») که هر دو آیتم را معرفی کند و اهمیت انتخاب درست را نشان دهد
   - جدول مقایسه سریع در ابتدای مقاله که خلاصه‌ای از مقایسه دو آیتم بر اساس معیارهای اصلی را نشان دهد
   - مقایسه تفصیلی بر اساس حداقل ۷ معیار مهم، هر کدام با یک تیتر H2 (با فرمت ## [معیار مقایسه])
   - در هر بخش مقایسه، عملکرد هر دو آیتم را بررسی کن و برنده آن بخش را با دلیل مشخص کن
   - بخش «چه زمانی $item1 را انتخاب کنیم؟» با توضیح مشخص برای چه کاربران یا شرایطی مناسب است
   - بخش «چه زمانی $item2 را انتخاب کنیم؟» با توضیح مشخص برای چه کاربران یا شرایطی مناسب است
   - جمع‌بندی و توصیه نهایی بر اساس سناریوهای مختلف (بدون اعلام یک برنده قطعی برای همه)
   
   توجه: فهرست مطالب را در ابتدای مقاله قرار نده، فقط محتوای اصلی تولید کن.

3. نکات مهم نگارشی:
   - حفظ بی‌طرفی کامل در سراسر متن
   - ذکر نقاط قوت و ضعف هر دو آیتم به صورت منصفانه
   - استفاده از داده‌ها، آمار و تست‌های عملکردی برای پشتیبانی از ادعاها
   - استفاده از جملات کوتاه و پاراگراف‌های کم‌حجم
   - استفاده از لیست‌های شماره‌دار و بولت‌پوینت برای خوانایی بهتر
   - استفاده از کلمات کلیدی مرتبط مانند «$item1 در مقابل $item2»، «مقایسه $item1 و $item2»، «تفاوت‌های $item1 و $item2»
   - متن‌های انگلیسی را چپ‌چین کن و از علامت‌های --- استفاده نکن

4. بهینه‌سازی محتوا:
   - استفاده از کلمات کلیدی مرتبط به صورت طبیعی
   - استفاده از هدینگ‌های H2 برای بخش‌های اصلی مقاله (با فرمت ## عنوان) و H3 برای زیربخش‌ها (با فرمت ### عنوان) به صورت سلسله مراتبی
   - پیشنهاد برای افزودن تصاویر مقایسه‌ای
   - پیشنهاد برای لینک‌دهی داخلی و خارجی به منابع مرتبط
   
5. نکته مهم: در ابتدای مقاله از عبارت «مقدمه» استفاده نکن. مستقیماً با یک پاراگراف جذاب شروع کن که هر دو آیتم را معرفی کند و اهمیت انتخاب درست را نشان دهد.

لطفاً محتوایی تولید کن که کاملاً طبیعی به نظر برسد و مانند یک مقاله مقایسه‌ای واقعی نوشته شده توسط یک متخصص بی‌طرف باشد.";
...
2. بدنه مقاله باید شامل این بخش‌های اصلی باشد (هر بخش باید با تیتر H2 مشخص شود، با فرمت ## عنوان):
   - معرفی $energy_topic (تاریخچه، اهمیت، کاربردها)
   - مبانی علمی و فنی (اصول، مفاهیم، تئوری‌ها)
   - تجهیزات و تکنولوژی‌های مورد استفاده
   - فرآیندها و روش‌های عملیاتی
...
3. نکات مهم نگارشی:
   - از کلمات کلیدی مرتبط با $energy_topic به صورت طبیعی استفاده کن
   - از عبارات و اصطلاحات تخصصی صنعت انرژی استفاده کن
   - از لحن تخصصی اما قابل فهم استفاده کن
   - از آمار و ارقام معتبر استفاده کن
   - از جملات کوتاه و پاراگراف‌های کم‌حجم استفاده کن
   - از لیست‌های شماره‌دار و بولت‌پوینت برای خوانایی بهتر استفاده کن
   - از نمودارها و دیاگرام‌های توضیحی استفاده کن
   - به نکات ایمنی و محیط زیستی اشاره کن

4. بهینه‌سازی محتوا:
   - طول مقاله: حداقل ۴۰۰۰ کلمه
   - استفاده از هدینگ‌های H2 برای بخش‌های اصلی مقاله (با فرمت ## عنوان) و H3 برای زیربخش‌ها (با فرمت ### عنوان) به صورت سلسله مراتبی
   - پیشنهاد برای افزودن تصاویر از تجهیزات، نمودارها، دیاگرام‌ها
   - پیشنهاد برای لینک‌دهی داخلی و خارجی به منابع مرتبط
   
5. نکته مهم: در ابتدای مقاله از عبارت «مقدمه» یا هر عنوان مشابه استفاده نکن. مستقیماً با یک پاراگراف جذاب شروع کن که خواننده را به ادامه مطلب ترغیب کند.

لطفاً محتوایی تولید کن که کاملاً طبیعی به نظر برسد و مانند یک مقاله تخصصی واقعی نوشته شده توسط یک متخصص صنعت انرژی باشد.";
        ...
        $prompt = "ایجاد یک مقاله تخصصی جامع و کاربردی در مورد $oil_topic با رعایت اصول سئو و بهینه‌سازی برای رنک مث.
...
ساختار مقاله:
1. عنوان اصلی (H1): $oil_topic: راهنمای تخصصی صنعت نفت و گاز (یا عنوانی جذاب دیگر)

2. بدنه مقاله باید شامل این بخش‌های اصلی باشد (هر بخش باید با تیتر H2 مشخص شود، با فرمت ## عنوان):
   - معرفی $oil_topic (تاریخچه، اهمیت، کاربردها)
   - مبانی علمی و زمین‌شناسی
   - تکنولوژی‌ها و تجهیزات مورد استفاده
   - فرآیندهای استخراج و پالایش
   - استانداردها و مقررات صنعتی
   - ایمنی و محیط زیست
   - بهینه‌سازی و مدیریت تولید
   - چالش‌ها و راه‌حل‌ها
   - روندهای آینده و نوآوری‌ها
   - مطالعات موردی و نمونه‌های عملی
   - پرسش‌های متداول (حداقل ۸ سوال و پاسخ)
...
3. نکات مهم نگارشی:
   - از کلمات کلیدی مرتبط با $oil_topic به صورت طبیعی استفاده کن
   - از عبارات و اصطلاحات تخصصی صنعت نفت و گاز استفاده کن
   - از لحن تخصصی اما قابل فهم استفاده کن
   - از آمار و ارقام معتبر استفاده کن
   - از جملات کوتاه و پاراگراف‌های کم‌حجم استفاده کن
   - از لیست‌های شماره‌دار و بولت‌پوینت برای خوانایی بهتر استفاده کن
...
لطفاً محتوایی تولید کن که کاملاً طبیعی به نظر برسد و مانند یک مقاله تخصصی واقعی نوشته شده توسط یک متخصص صنعت نفت و گاز باشد.";
        ...
        $prompt = "ایجاد یک مقاله تخصصی جامع و کاربردی در مورد $fire_topic با رعایت اصول سئو و بهینه‌سازی برای رنک مث.
...
ساختار مقاله:
1. عنوان اصلی (H1): $fire_topic: راهنمای تخصصی ایمنی و آتشنشانی (یا عنوانی جذاب دیگر)

2. بدنه مقاله باید شامل این بخش‌های اصلی باشد (هر بخش باید با تیتر H2 مشخص شود، با فرمت ## عنوان):
   - معرفی $fire_topic (اهمیت، کاربردها، ضرورت)
   - مبانی علمی و تئوری آتش
   - تجهیزات و تکنولوژی‌های مورد استفاده
   - روش‌ها و پروتکل‌های عملیاتی
   - استانداردها و مقررات ایمنی
   - آموزش و آمادگی پرسنل
...
3. نکات مهم نگارشی:
   - از کلمات کلیدی مرتبط با $fire_topic به صورت طبیعی استفاده کن
   - از عبارات و اصطلاحات تخصصی آتشنشانی استفاده کن
   - از لحن تخصصی اما قابل فهم استفاده کن
   - از آمار و ارقام معتبر استفاده کن
   - از جملات کوتاه و پاراگراف‌های کم‌حجم استفاده کن
   - از لیست‌های شماره‌دار و بولت‌پوینت برای خوانایی بهتر استفاده کن
   - از نمودارها و دیاگرام‌های توضیحی استفاده کن
   - به نکات ایمنی و پیشگیری اشاره کن
...
لطفاً محتوایی تولید کن که کاملاً طبیعی به نظر برسد و مانند یک مقاله تخصصی واقعی نوشته شده توسط یک متخصص آتشنشانی باشد.";
        
    } else {
        // قالب پیش‌فرض
        $main_topic = isset($form_data['main_topic']) ? sanitize_text_field($form_data['main_topic']) : 'موضوع مقاله';
        
        $prompt = "ایجاد یک مقاله جامع و با کیفیت در مورد $main_topic با رعایت اصول سئو.

موضوع: $main_topic
طول مقاله: حداقل ۱۵۰۰ کلمه

ساختار مقاله:
1. عنوان اصلی (H1): عنوانی جذاب و گیرا که موضوع اصلی را به خوبی منعکس کند

2. بدنه مقاله باید شامل این بخش‌ها باشد:
   - پاراگراف شروعی جذاب (بدون استفاده از عنوان «مقدمه») که موضوع را معرفی کند و اهمیت آن را نشان دهد
   - حداقل ۵ بخش اصلی که هر کدام با تیتر H2 مشخص شود (با فرمت ## عنوان) و جنبه‌های مختلف موضوع را پوشش دهند
   - زیربخش‌های مرتبط با H3 (با فرمت ### عنوان) برای هر بخش اصلی
   - استفاده از مثال‌ها و موارد کاربردی
   - بخش پرسش و پاسخ (FAQ) با سوالات متداول
   - جمع‌بندی و نتیجه‌گیری
   
   توجه: فهرست مطالب را در ابتدای مقاله قرار نده، فقط محتوای اصلی تولید کن.

3. نکات مهم نگارشی:
   - استفاده از لحن حرفه‌ای و در عین حال قابل فهم
   - استفاده از جملات کوتاه و پاراگراف‌های کم‌حجم
   - استفاده از لیست‌های شماره‌دار و بولت‌پوینت برای خوانایی بهتر
   - استفاده از آمار، ارقام و نقل قول‌های معتبر
   - استفاده از مثال‌های عملی و کاربردی

4. بهینه‌سازی محتوا:
   - استفاده از کلمات کلیدی مرتبط با $main_topic به صورت طبیعی
   - استفاده از هدینگ‌های H2 برای بخش‌های اصلی مقاله (با فرمت ## عنوان) و H3 برای زیربخش‌ها (با فرمت ### عنوان) به صورت سلسله مراتبی
   - پیشنهاد برای افزودن تصاویر مرتبط
   - پیشنهاد برای لینک‌دهی داخلی و خارجی به منابع مرتبط
   
5. نکته مهم: در ابتدای مقاله از عبارت «مقدمه» استفاده نکن. مستقیماً با یک پاراگراف جذاب شروع کن که موضوع را معرفی کند و خواننده را به ادامه مطلب ترغیب کند.

لطفاً محتوایی تولید کن که کاملاً طبیعی به نظر برسد و مانند یک مقاله واقعی نوشته شده توسط یک متخصص در این حوزه باشد.";
    }
